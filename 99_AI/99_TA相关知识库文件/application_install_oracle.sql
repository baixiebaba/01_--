...
COL userL NEW_VALUE userV NOPRINT;
SELECT TO_CHAR(SYSDATE, 'YYYY_MM_DD_HH24_MI_SS') timestamp_heading,
          user userL
FROM DUAL;

spool application_install_oracle_&timestamp&userV;
...
-- Checks. If success, run application_install_oracle_sub.sql
SET VERIFY OFF;
...
SELECT
  ...
  (CASE WHEN SUBSTR(VERSION, 1, INSTR(VERSION,'.')-1) < 10
        THEN '1'
        ELSE 'application_install_oracle_sub.sql'
   END) AS col_errore,
  (CASE WHEN SUBSTR(VERSION, 1, INSTR(VERSION,'.')-1) < 10
        THEN '1'
        ELSE '0'
