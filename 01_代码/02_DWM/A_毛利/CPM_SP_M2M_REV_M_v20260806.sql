...
修改日志：
  ...
  20260717 shiqingfeng.ex 取消插入墨西哥利润调整
  20260730 SHQINGFENG.EX 新增 MA_SALE_COGS 字段
  20260804 SHIQINGFENG.EX 补充2900公司取数时排除ZZFT
  20260806 【需求】管报中国区(1180、12**公司)政策分摊兜底逻辑：D_CHANNEL替换为D_ECOM_BU；D_ECOM_BU为空或ZZZZ不分摊；不跨公司分摊
  20260806 【fix】修复v1两处BUG：(1)1.1不应直接排除D_ECOM_BU空/ZZZZ记录→改为留入TMP_TABLE_POLICY+A_STEP3收入端排除+自然落入1.4未分配政策；(2)1.4未分配政策JOIN中M端D_CHANNEL未做转换→补CASE WHEN
...
;
  ...
  INTO V_MATERIAL
    ...
    VALUES('CPM_SP_M2M_REV_M', '0 START', SYSDATE, SESSION_USER, P_SCENARIO, P_PERIODO, '大系统产销存销售、收入、成本计算开始',P_AZIENDA);
    COMMIT;
  ...
  ------------------------------------------------------------------------------
    /*
  DROP TABLE TMP_TABLE_POLICY ;
    CREATE GLOBAL TEMPORARY TABLE TMP_TABLE_POLICY(
        COD_AZIENDA     VARCHAR2(10),
        COD_DEST2       VARCHAR2(30),
        D_ONOFFLINE     VARCHAR2(30),
        D_CHANNEL       VARCHAR2(30),
        CUST_CODE       VARCHAR2(30),
        MATERIAL_CODE   VARCHAR2(30),
    COD_AZIENDA_NEW VARCHAR2(30),
        POLICY_AMT      NUMBER,
        STEP_ID         VARCHAR2(20),
    ...
    */
    ...
    /*
  DROP TABLE TMP_TABLE_POLICY_ALLOC ;
    CREATE GLOBAL TEMPORARY TABLE TMP_TABLE_POLICY_ALLOC(
        OID             VARCHAR2(36) NOT NULL ENABLE,
        STEP_ID         VARCHAR2(20),
        GROUP_ID        VARCHAR2(100),
        MAP_REV1         NUMBER,
        POLICY_AMT      NUMBER,
        POLICY_ALC_AMT  NUMBER
    )
    ON COMMIT DELETE ROWS ;
    */

    /*
    CREATE GLOBAL TEMPORARY TABLE TMP_TABLE_ENT(
        COD_CTP            VARCHAR2(10) NOT NULL ENABLE,
        COD_BU         VARCHAR2(10)
    )
    ON COMMIT DELETE ROWS ;
    */
  ...
  -----------------------------------------------------------------------------
    ...
    --插入分公司政策数据到临时表
  INSERT INTO TMP_TABLE_POLICY(COD_AZIENDA, COD_DEST2, D_ONOFFLINE, D_CHANNEL, CUST_CODE, MATERIAL_CODE, COD_AZIENDA_NEW, POLICY_AMT, STEP_ID, GROUP_ID)
    --非空调
  SELECT
     --【20260806 修改】 D_CHANNEL→D_ECOM_BU(1180/12**)
        COD_AZIENDA, COD_DEST2, NVL(D_ONOFFLINE,'') D_ONOFFLINE, CASE WHEN (COD_AZIENDA = '1180' OR COD_AZIENDA LIKE '12%') THEN NVL(D_ECOM_BU,'') ELSE NVL(D_CHANNEL,'') END D_CHANNEL, NVL(CUST_CODE,'') CUST_CODE,
        NULL MATERIAL_CODE, NULL COD_AZIENDA_NEW, SUM(POLICY_AMT) POLICY_AMT, NULL STEP_ID, NULL GROUP_ID
    FROM TGK_FIMA_HISENSE.AW_MR9_REVM01_000001
   WHERE SRC_DETAIL IN ('POLICY_YT','ADJM01')
     ...
     --【20260806 修改v2】 不再在1.1排除D_ECOM_BU为空/ZZZZ的记录，改为在A_STEP3收入端排除，让其为"未分配政策"
     /*20260226 新增排除锁定公司*/
     ...
     --【20260806 修改】 D_CHANNEL→D_ECOM_BU(1180/12**)
    GROUP BY COD_AZIENDA, COD_DEST2, NVL(D_ONOFFLINE,''), CASE WHEN (COD_AZIENDA = '1180' OR COD_AZIENDA LIKE '12%') THEN NVL(D_ECOM_BU,'') ELSE NVL(D_CHANNEL,'') END, NVL(CUST_CODE,'')
  ...
  --空调
    SELECT
     --【20260806 修改】 D_CHANNEL→D_ECOM_BU(1180/12**)
        COD_AZIENDA, D_SALE_DEPT AS COD_DEST2, NVL(D_ONOFFLINE,'') D_ONOFFLINE, CASE WHEN (COD_AZIENDA = '1180' OR COD_AZIENDA LIKE '12%') THEN NVL(D_ECOM_BU,'') ELSE NVL(D_CHANNEL,'') END D_CHANNEL, NVL(CUST_CODE,'') CUST_CODE,
        NULL MATERIAL_CODE, NULL COD_AZIENDA_NEW, SUM(POLICY_AMT) POLICY_AMT, NULL STEP_ID, NULL GROUP_ID
    FROM TGK_FIMA_HISENSE.AW_MR9_REVM01_000001
   WHERE SRC_DETAIL IN ('POLICY_YT','ADJM01')
     AND COD_CONTO IN ('S6001G5','S6001G5AD')
     ...
     --【20260806 修改】 D_CHANNEL→D_ECOM_BU(1180/12**)
    GROUP BY COD_AZIENDA, D_SALE_DEPT, NVL(D_ONOFFLINE,''), CASE WHEN (COD_AZIENDA = '1180' OR COD_AZIENDA LIKE '12%') THEN NVL(D_ECOM_BU,'') ELSE NVL(D_CHANNEL,'') END, NVL(CUST_CODE,'')
    HAVING SUM(POLICY_AMT) <> 0
    ;

    --插入公司对应的事业部关系进入临时表
    INSERT INTO TMP_TABLE_ENT(COD_CTP, COD_BU)
    SELECT COD_AZIENDA COD_CTP, 'BX' COD_BU
    FROM AZIENDA
    WHERE COD_AZIENDA LIKE '63%' OR COD_AZIENDA LIKE '67%'
    ;

    INSERT INTO TMP_TABLE_ENT(COD_CTP, COD_BU)
    SELECT COD_AZIENDA COD_CTP, 'KT' COD_BU
    FROM AZIENDA
    WHERE COD_AZIENDA LIKE '62%' OR COD_AZIENDA LIKE '68%'
    ;

    INSERT INTO TMP_TABLE_ENT(COD_CTP, COD_BU)
    SELECT COD_AZIENDA COD_CTP, 'SX' COD_BU
    FROM AZIENDA
    WHERE COD_AZIENDA LIKE '2%' OR COD_AZIENDA LIKE '163%'
    ;
    DBMS_OUTPUT.put_line('1.2A  处理分公司及总部线上政策数据');
  ------------------------------------------------------------------------------
  -- 1.2A  处理分公司及总部线上政策数据
  ------------------------------------------------------------------------------
...
;
 ...
 ;
    ...
    --扩展物料编码
    INSERT INTO TMP_TABLE_POLICY(D_CHANNEL,COD_AZIENDA, COD_DEST2, D_ONOFFLINE, CUST_CODE, MATERIAL_CODE, COD_AZIENDA_NEW, POLICY_AMT, STEP_ID, GROUP_ID)
    SELECT
        DISTINCT M.D_CHANNEL,M.COD_AZIENDA, M.COD_DEST2, M.D_ONOFFLINE, M.CUST_CODE, A1.MATERIAL_CODE, NULL COD_AZIENDA_NEW, M.POLICY_AMT, 'A_STEP2' STEP_ID, M.GROUP_ID
   ...
   ;
    --剔除原来STEP2的准备记录
    DELETE FROM TMP_TABLE_POLICY M WHERE M.STEP_ID IN ('A_STEP2_READY') ;
    ...
    WHERE EXISTS(SELECT 1
                  FROM (
                        --非空调
     --【20260806 修改】 D_CHANNEL→D_ECOM_BU(1180/12**)
                        SELECT DISTINCT COD_AZIENDA, LTRIM(COD_DEST2,'0') COD_DEST2, D_ONOFFLINE, CASE WHEN (COD_AZIENDA = '1180' OR COD_AZIENDA LIKE '12%') THEN D_ECOM_BU ELSE D_CHANNEL END D_CHANNEL
                          FROM TGK_FIMA_HISENSE.AW_MR9_REVM01_000001
                     LEFT JOIN TMP_TABLE_ENT
                            ON (CASE WHEN COD_AZIENDA LIKE '63%' OR COD_AZIENDA LIKE '67%' THEN 'BX'
                                     WHEN COD_AZIENDA LIKE '62%' OR COD_AZIENDA LIKE '68%' THEN 'KT'
                          ...
                          WHERE COD_SCENARIO = P_SCENARIO
                            ...
                            AND INSTR(V_PRO_CLASS, SUBSTR(NVL(D_PRO_CLASS,'|'),1,5)) = 0
                            AND NVL(MATERIAL_GROUP_CODE,'|') NOT IN ('1100105','1100107')
     --【20260806 修改】 1180/12**公司D_ECOM_BU为空或ZZZZ的不分摊
                            AND NOT ((COD_AZIENDA = '1180' OR COD_AZIENDA LIKE '12%') AND NVL(D_ECOM_BU,'ZZZZ') = 'ZZZZ')
                            --限制内销
                            AND COD_DEST3 LIKE '2%'
                            /*20260226 新增排除锁定公司*/
                            AND COD_AZIENDA NOT IN (SELECT COD_AZIENDA
                           ...
                           AND NVL(SRC_DETAIL,'|') <> 'REVM01'
     --【20260806 修改】 D_CHANNEL→D_ECOM_BU(1180/12**)
                      GROUP BY COD_AZIENDA, LTRIM(COD_DEST2,'0') , D_ONOFFLINE, CASE WHEN (COD_AZIENDA = '1180' OR COD_AZIENDA LIKE '12%') THEN D_ECOM_BU ELSE D_CHANNEL END
                        HAVING ROUND(SUM(MAP_REV1),2) <> 0

                    UNION ALL
                        --空调
     --【20260806 修改】 D_CHANNEL→D_ECOM_BU(1180/12**)
                        SELECT DISTINCT COD_AZIENDA, LTRIM(D_SALE_DEPT,'0') COD_DEST2, D_ONOFFLINE, CASE WHEN (COD_AZIENDA = '1180' OR COD_AZIENDA LIKE '12%') THEN D_ECOM_BU ELSE D_CHANNEL END D_CHANNEL
                          FROM TGK_FIMA_HISENSE.AW_MR9_REVM01_000001
                     LEFT JOIN TMP_TABLE_ENT
                          ON (CASE WHEN COD_AZIENDA LIKE '63%' OR COD_AZIENDA LIKE '67%' THEN 'BX'
                                   WHEN COD_AZIENDA LIKE '62%' OR COD_AZIENDA LIKE '68%' THEN 'KT'
                       ...
                       WHERE COD_SCENARIO = P_SCENARIO
                         ...
                         AND NVL(SRC_DETAIL,'|') <> 'REVM01'
     --【20260806 修改】 D_CHANNEL→D_ECOM_BU(1180/12**)
                    GROUP BY COD_AZIENDA, LTRIM(D_SALE_DEPT,'0') , D_ONOFFLINE, CASE WHEN (COD_AZIENDA = '1180' OR COD_AZIENDA LIKE '12%') THEN D_ECOM_BU ELSE D_CHANNEL END
                      HAVING ROUND(SUM(MAP_REV1),2) <> 0
                     ) A1
              WHERE M.COD_AZIENDA = A1.COD_AZIENDA
                AND M.COD_DEST2 = A1.COD_DEST2
...
DBMS_OUTPUT.put_line('1.2B  处理总公司线下政策数据');
...
;
    ...
    INSERT INTO TMP_TABLE_POLICY(COD_AZIENDA, COD_DEST2, D_ONOFFLINE, D_CHANNEL, CUST_CODE, MATERIAL_CODE, COD_AZIENDA_NEW, POLICY_AMT, STEP_ID, GROUP_ID)
    SELECT
        DISTINCT M.COD_AZIENDA, M.COD_DEST2, M.D_ONOFFLINE, M.D_CHANNEL, M.CUST_CODE, NULL MATERIAL_CODE, A1.COD_AZIENDA_NEW, M.POLICY_AMT, 'B_STEP1' STEP_ID, M.GROUP_ID
    FROM TMP_TABLE_POLICY M
    JOIN (
          --非空调
...
;
    ...
    --剔除原来STEP1的准备记录
    DELETE FROM TMP_TABLE_POLICY M WHERE M.STEP_ID IN ('B_STEP1_READY') ;
 ...
 ;
    ...
    INSERT INTO TMP_TABLE_POLICY(COD_AZIENDA, COD_DEST2, D_ONOFFLINE, D_CHANNEL, CUST_CODE, MATERIAL_CODE, COD_AZIENDA_NEW, POLICY_AMT, STEP_ID, GROUP_ID)
    SELECT
        DISTINCT M.COD_AZIENDA, M.COD_DEST2, M.D_ONOFFLINE, M.D_CHANNEL, M.CUST_CODE, NULL MATERIAL_CODE, A1.COD_AZIENDA_NEW, M.POLICY_AMT, 'B_STEP2' STEP_ID, M.GROUP_ID
    FROM TMP_TABLE_POLICY M
    JOIN
          (
          --非空调
...
;
  ...
  ------------------------------------------------------------------------------
    ...
    --在每条收入明细上，生成对应的分摊数据
    INSERT INTO TMP_TABLE_POLICY_ALLOC
    SELECT
        OID, STEP_ID, GROUP_ID, MAP_REV1, POLICY_AMT
    ...
    FROM(
     ...
     --【20260806 修改】 D_CHANNEL→D_ECOM_BU(1180/12**)
                         D_ONOFFLINE, CASE WHEN (COD_AZIENDA = '1180' OR COD_AZIENDA LIKE '12%') THEN D_ECOM_BU ELSE D_CHANNEL END D_CHANNEL, CUST_CODE, MATERIAL_CODE, COD_AZI_CTP,MAP_REV1
...
;
  ...
  INSERT INTO TMP_TABLE_POLICY_ALLOC(OID,STEP_ID,GROUP_ID,MAP_REV1,POLICY_AMT,POLICY_ALC_AMT)
    --本月
  SELECT
        OID, 'CURRENT_PERIOD' STEP_ID, NULL GROUP_ID, NULL MAP_REV1, NULL POLICY_AMT, SUM(POLICY_ALC_AMT) POLICY_ALC_AMT
         
    FROM TMP_TABLE_POLICY_ALLOC
    WHERE STEP_ID IS NOT NULL AND STEP_ID <> 'A_STEP2'
  GROUP BY OID 

  UNION ALL
  --上2个月
  SELECT
        OID, 'LAST_TWO_PERIOD' STEP_ID, NULL GROUP_ID, NULL MAP_REV1, NULL POLICY_AMT, SUM(POLICY_ALC_AMT) POLICY_ALC_AMT
    FROM TMP_TABLE_POLICY_ALLOC
    WHERE STEP_ID IS NOT NULL AND STEP_ID = 'A_STEP2'
    GROUP BY OID 
     ;
    ...
    -- 1.3 生成并汇总政策分摊后的数据
    INSERT INTO ZTAB_CPM_LOG(CPM, STEP, EXECTIME, CREATEBY, COD_SCENARIO, COD_PERIODO, REMARK,COD_AZIENDA)
    VALUES('CPM_SP_M2M_REV_M', '1 单体-收入、政策', SYSDATE, SESSION_USER, P_SCENARIO, P_PERIODO, '1.3 生成并汇总政策分摊后的数据',P_AZIENDA);
    --COMMIT;
  ...
  ------------------------------------------------------------------------------
    ...
    FROM TGK_FIMA_HISENSE.AW_MR9_REVM01_000001 M
    JOIN (SELECT * FROM TMP_TABLE_POLICY A11 WHERE STEP_ID IS NULL 
              OR EXISTS 
                (SELECT 1 FROM TMP_TABLE_POLICY_ALLOC A22 
                WHERE A11.STEP_ID = A22.STEP_ID 
     ...
     --【20260806 修改v2】 M端D_CHANNEL也做D_ECOM_BU转换，保证与A1端一致
     AND NVL(CASE WHEN (M.COD_AZIENDA = '1180' OR M.COD_AZIENDA LIKE '12%') THEN M.D_ECOM_BU ELSE M.D_CHANNEL END,'Z') = NVL(A1.D_CHANNEL,'Z')
...
DELETE FROM TMP_TABLE_POLICY_ALLOC 
 ...
 ;*/
    ...
    FROM TGK_FIMA_HISENSE.AW_MR9_REVM01_000001 M
  --LEFT JOIN， 要COPY所有记录
    LEFT JOIN (SELECT * FROM TMP_TABLE_POLICY_ALLOC WHERE STEP_ID = 'CURRENT_PERIOD' )A1
           ON M.OID = A1.OID
        WHERE 1=1
  ...
  --注意这里要用inner join， 只COPY匹配记录
    JOIN (SELECT * FROM TMP_TABLE_POLICY_ALLOC WHERE STEP_ID = 'LAST_TWO_PERIOD' )A1
      ON M.OID = A1.OID
    WHERE 1=1
      AND M.COD_CONTO LIKE 'S60010%'
      --AND M.COD_AZIENDA NOT LIKE '8%'
