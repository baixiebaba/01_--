/*
-- ============================================================================
-- 测试脚本：WRITEOFF 核销来源窄事实核对
-- 对应来源：dwd_fi_mr_arap_sum_mi.sql 的 WRITEOFF 分支
-- 源表：核销单据、核销子表、组织/期间/科目/客商等 ODS 表及科目映射规则表
-- 金额口径：分别汇总 fk_hifi_local_lossmoneys 本位币和 fk_hifi_lossmoneys 交易币，非零过滤
-- 最终输出：writeoff_fact
-- ============================================================================
*/
...
WITH
-- 依据当月DETAIL按公司和客商去重性质，供核销事实回接，避免金额放大。
detail_nature AS (
...
-- WRITEOFF：保留审核状态、SAP凭证号和指定科目范围，按业务维度汇总金额。
writeoff_sum AS (
...
-- WRITEOFF：标准化为窄事实接口，账龄段固定为1并过滤双币种均为零的记录。
writeoff_fact AS (
...
-- 来源级Sum聚合：按全部非金额字段汇总WRITEOFF账龄金额，结果不是窄事实明细。
writeoff_fact_sum AS (
    SELECT dt_month
         , `year`
         , `month`
         , company_code
...
-- 测试目的：核对WRITEOFF来源级Sum聚合结果，而非窄事实明细。
SELECT *
  FROM writeoff_fact_sum;
