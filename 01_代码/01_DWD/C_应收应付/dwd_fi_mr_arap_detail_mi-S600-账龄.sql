/*
-- ============================================================================
-- 最新版修改记录：20260910 MOD BY Kiro 最外层增加公司筛选及本位币、交易币金额汇总
...
-- ============================================================================
-- 第一段：统驭科目凭证行项目。
-- 仅处理S600的BSID/BSAD/BSIK/BSAK；本段为查询测试，不写入目标表。
-- ============================================================================
-- 按公司汇总账龄和非统驭余额，本位币和交易币分别汇总。
SELECT SUM(a.bcy_amt) AS bcy_amt
     , SUM(a.qcy_amt) AS qcy_amt
  FROM (
WITH
-- 汇总S600应收应付科目范围配置。
src_ztzt_003 AS (
