...
TABLES:sscrfields,bsis,bsik,bsid,skb1,t001,zath_check,bkpf.
TYPE-POOLS:icon.
...
*      dmbtr LIKE bsik-dmbtr,"本位币金额 DEL BY WANGL AT 20240705
      dmbtr TYPE mc_dmbtr,"本位币金额 ADD BY WANGL AT 20240705
      waers LIKE bsik-waers,"交易币币种
*      wrbtr LIKE bsik-wrbtr,"交易币金额 DEL BY WANGL AT 20240705
      wrbtr TYPE udm_open_item_amt,"交易币金额 ADD BY WANGL AT 20240705
...
*{ -------------------------------------------------------------202409 add
      ...
      projk_ext TYPE projk_ext,
*} -------------------------------------------------------------202409 add
  END OF gt_bskd.
DATA:wa_bskd LIKE LINE OF gt_bskd.
...
*************************************************20211213 ADD LIXIANGFENG
DATA:gt_bukrs LIKE STANDARD TABLE OF t001-bukrs.
*************************************************20211213 ADD LIXIANGFENG
...
*************************************************20211213 DEL LIXIANGFENG
*  PERFORM save_check USING ''.
*************************************************20211213 DEL LIXIANGFENG
...
*************************************************20211213 ADD LIXIANGFENG
*************************************************20211213 MOD LIXIANGFENG
*  SELECT bukrs INTO gv_bukrs
*    FROM t001
*    WHERE bukrs IN s_bukrs.
  LOOP AT gt_bukrs INTO gv_bukrs.
*************************************************20211213 MOD LIXIANGFENG
...
FORM get_data .
  ...
  SELECT * INTO CORRESPONDING FIELDS OF TABLE lt_003
    ...
    WHERE t001~bukrs = gv_bukrs.

  LOOP AT lt_003.
**判断是否统驭科目，配置表中，同一行不能统驭科目、非统驭科目并存，要拆开配置
...
*  IF p_pzrq = 'X'."凭证日期
*    whrstr = whrstr && ` and bldat <= p_keydat `.
*  ELSEIF p_jzrq = 'X'.
*    whrstr = whrstr && ` and zfbdt <= p_keydat`.
*  ENDIF.
...
*          blart <> 'DA' AND "凭证类型为DA的排除掉
         bstat <> 'A'  AND "凭证状态为A的排除掉
         bstat <> 'S'  AND "凭证状态为S的排除掉
         (whrstr).

  tabnam+2(1) = 'A'.
  whrstr = whrstr && ` and augdt > p_keydat`.
  SELECT *
    APPENDING CORRESPONDING FIELDS OF TABLE gt_bskd
    FROM (tabnam)
    WHERE bukrs = gv_bukrs AND
...
***&&& CHANGGE BY WANGYL 20241223 END}
  ...
  SELECT SINGLE * FROM t001 WHERE bukrs = gv_bukrs.
  LOOP AT gt_bskd.
*{ -------------------------------------------------------------202409 add
    IF gt_bskd-projk IS NOT INITIAL.
      CALL FUNCTION 'CONVERSION_EXIT_ABPSP_OUTPUT'
        EXPORTING
          input  = gt_bskd-projk
        IMPORTING
          output = gt_bskd-projk_ext.
    ENDIF.
*} -------------------------------------------------------------202409 add
    ...
    IF gt_bskd-blart = 'AB'. "AB类凭证基于基准日期计算账龄，其他凭证基选择条件计算
      lv_datum = gt_bskd-zfbdt.
    ENDIF.
...
***&&& CHANGGE BY WANGYL 20241223 END}
*{ -------------------------------------------------------------202409 add
      itab-projk_ext = gt_bskd-projk_ext.
*} -------------------------------------------------------------202409 add
...
*{ -------------------------------------------------------------202409 add
                          'PROJK_EXT' 'BSEG_ALV' 'PROJK_EXT' 'WBS元素',
*} -------------------------------------------------------------202409 add
                          'KEYDAT' '' '' '关键日期',
...
FORM catlg_set_days  USING    value(p_text1)
  ...
  lv_text = p_text1 && '账龄从' && lv_days1 && '到' && lv_days2 && '天'.
...
FORM save_zzt003 .
  ...
  DELETE FROM zzt003_save WHERE bukrs = gv_bukrs AND koart = g_koart AND keydat = p_keydat.
  COMMIT WORK.
