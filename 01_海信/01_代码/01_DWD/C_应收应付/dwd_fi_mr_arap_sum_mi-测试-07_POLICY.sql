/*
-- ============================================================================
-- 测试脚本：POLICY 政策欠付返利来源窄事实核对
-- 对应来源：dwd_fi_mr_arap_sum_mi.sql 的 POLICY 分支
-- 源表：ods.ods_plc_v_hpms_account_gb 及当月 DETAIL 性质
-- 金额口径：按期间、公司、客户、利润中心和政策状态汇总 arrears_amount，并过滤非零金额
-- 最终输出：policy_fact
-- ============================================================================
*/
...
WITH
-- 依据当月DETAIL按公司和客商去重性质，供POLICY事实回接，避免金额放大。
detail_nature AS (
...
-- POLICY：按期间、公司、客户、利润中心和政策状态汇总欠付返利。
policy_sum AS (
...
-- POLICY：标准化为窄事实接口，保留政策状态、账龄段1和非零过滤。
policy_fact AS (
...
-- 来源级Sum聚合：按全部非金额字段汇总POLICY账龄金额，结果不是窄事实明细。
policy_fact_sum AS (
    SELECT dt_month
         , `year`
         , `month`
...
-- 测试目的：核对POLICY来源级Sum聚合结果，而非窄事实明细。
SELECT *
  FROM policy_fact_sum;
