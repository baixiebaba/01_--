/*
-- ============================================================================
-- 测试脚本：XH_AR 信汇应收来源窄事实核对
-- 对应来源：dwd_fi_mr_arap_sum_mi.sql 的 XH 应收分支
-- 源表：ods.odsmt_fin_bill_view_data、ODS公司/账户映射表及当月 DETAIL 性质
-- 金额口径：票据 bill_amount 汇总为应收正向金额，非零过滤
-- 最终输出：xh_ar_fact
-- ============================================================================
*/
...
-- 来源级Sum聚合：按全部非金额字段汇总XH应收账龄金额，结果不是窄事实明细。
xh_ar_fact_sum AS (
    SELECT dt_month
         , `year`
         , `month`
