/*
-- ============================================================================
-- 最新版修改记录：20260910 MOD BY Kiro 顶部新增公司筛选参数
-- 上一版修改记录：20260910 ADD BY shiqingfeng.ex 新增
-- 文件说明：包含统驭科目账龄及非统驭科目余额的完整明细查询测试脚本
-- 修改记录：最新修改记录放最上面
...
-- 以上SET与下方WITH ... SELECT查询必须在同一session中执行。

-- ============================================================================
-- 第一段：统驭科目凭证行项目。
-- 仅处理S700的BSID/BSAD/BSIK/BSAK；本段为查询测试，不写入目标表。
-- 完整账龄与非统驭余额结果统一按顶部公司参数筛选。
SELECT *
  FROM (
WITH
...
-- 汇总S700公司代码、科目表和本位币配置，并映射为标准公司编码。
src_t001 AS (
    SELECT 'S700' AS system_src
         , COALESCE(NULLIF(TRIM(company_map.cod_azienda), ''), t.bukrs) AS bukrs
         , t.ktopl
         , t.waers
      FROM ods.odss700_t001 t
      LEFT JOIN s700_company_map company_map
        ON company_map.ent_sap700_code = TRIM(t.bukrs)
),
-- 汇总S700公司科目及统驭标识，用于识别统驭/非统驭科目。
src_skb1 AS (
    SELECT 'S700' AS system_src
         , COALESCE(NULLIF(TRIM(company_map.cod_azienda), ''), s.bukrs) AS bukrs
         , s.saknr
         , s.mitkz
      FROM ods.odss700_skb1 s
      LEFT JOIN s700_company_map company_map
        ON company_map.ent_sap700_code = TRIM(s.bukrs)
),
-- 汇总S700客户主数据，提供客户名称和MDG编码。
src_kna1 AS (
    SELECT 'S700' AS system_src, kunnr, name1, zkunnr_mdg
      FROM ods.odss700_kna1
),
-- 汇总S700供应商主数据，提供供应商名称和MDG编码。
src_lfa1 AS (
    SELECT 'S700' AS system_src, lifnr, name1, zlifnr_mdg
      FROM ods.odss700_lfa1
),
-- 汇总S700公司级客商主数据KVERM，按系统、公司、客商类型和主户编码聚合非空付款条件。
cust_kverm_dim AS (
    SELECT k.system_src
         , k.company_code
         , k.cust_code
      ...
      FROM (
            SELECT 'S700' AS system_src
                 , COALESCE(NULLIF(TRIM(hm.cod_azienda), ''), k.bukrs) AS company_code
                 , LTRIM(k.kunnr, '0') AS cust_code
                 , 'C' AS cust_type_code
                 , NULLIF(TRIM(k.kverm), '') AS kverm
            ...
            SELECT 'S700' AS system_src
                 , COALESCE(NULLIF(TRIM(hm.cod_azienda), ''), l.bukrs) AS company_code
                 , LTRIM(l.lifnr, '0') AS cust_code
                 , 'V' AS cust_type_code
                 , NULLIF(TRIM(l.kverm), '') AS kverm
...
-- 汇总S700利润中心文本，用于补充利润中心名称。
src_cepct AS (
    SELECT 'S700' AS system_src, prctr, spras, ktext
      FROM ods.odss700_cepct
),
-- 客户未清项目：仅保留参数月末前已过账且非A/S状态的记录，并映射公司编码。
src_bsid AS (
    SELECT 'S700' AS system_src
         , 'BSID' AS ods_src
         , COALESCE(NULLIF(TRIM(company_map.cod_azienda), ''), b.bukrs) AS bukrs
         , b.kunnr AS cust_head_code
         , b.hkont
         , b.prctr
         , b.gsber
         , b.filkd
      ...
      LEFT JOIN s700_company_map company_map
        ON company_map.ent_sap700_code = TRIM(b.bukrs)
     WHERE b.budat <= @key_date_sap
       AND COALESCE(b.bstat, '') NOT IN ('A', 'S')
       AND (COALESCE(b.dmbtr, 0) <> 0 OR COALESCE(b.wrbtr, 0) <> 0)
),
-- 客户已清项目：保留月末前过账、月末后清账的项目，回溯月末未清快照，并映射公司编码。
src_bsad AS (
    SELECT 'S700' AS system_src
         , 'BSAD' AS ods_src
         , COALESCE(NULLIF(TRIM(company_map.cod_azienda), ''), b.bukrs) AS bukrs
      ...
      LEFT JOIN s700_company_map company_map
        ON company_map.ent_sap700_code = TRIM(b.bukrs)
     WHERE b.budat <= @key_date_sap
       AND b.augdt > @key_date_sap
       AND COALESCE(b.bstat, '') NOT IN ('A', 'S')
       AND (COALESCE(b.dmbtr, 0) <> 0 OR COALESCE(b.wrbtr, 0) <> 0)
),
-- 供应商未清项目：仅保留参数月末前已过账且非A/S状态的记录，并映射公司编码。
src_bsik AS (
    SELECT 'S700' AS system_src
         , 'BSIK' AS ods_src
         , COALESCE(NULLIF(TRIM(company_map.cod_azienda), ''), b.bukrs) AS bukrs
         , b.lifnr AS cust_head_code
         , b.hkont
         , b.prctr
         , b.gsber
         , b.filkd
...
-- 供应商已清项目：保留月末前过账、月末后清账的项目，回溯月末未清快照，并映射公司编码。
src_bsak AS (
    SELECT 'S700' AS system_src
         , 'BSAK' AS ods_src
         , COALESCE(NULLIF(TRIM(company_map.cod_azienda), ''), b.bukrs) AS bukrs
...
-- 汇总S700总账余额源，并在物理源处按参数财年过滤，同时映射公司编码。
src_gl_balance AS (
    SELECT 'S700' AS system_src
         , COALESCE(NULLIF(TRIM(company_map.cod_azienda), ''), g.rbukrs) AS company_code
         , g.racct AS acct_src_code
         , g.prctr AS src_profitcenter_code
         , g.rbusa AS bus_range_code
         , g.rtcur AS qcy_code
      ...
      LEFT JOIN s700_company_map company_map
        ON company_map.ent_sap700_code = TRIM(g.rbukrs)
     -- 仅扫描参数财年，避免读取无关年度余额。
     WHERE g.ryear = @fiscal_year
),
