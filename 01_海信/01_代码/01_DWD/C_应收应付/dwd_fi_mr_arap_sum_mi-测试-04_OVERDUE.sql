/*
-- ============================================================================
-- 测试脚本：OVERDUE 超期款来源窄事实核对
-- 对应来源：dwd_fi_mr_arap_sum_mi.sql 的 OVERDUE 分支
-- 源表：dws.dws_fi_mr_ar_overdue_mi 及当月 DETAIL 性质
-- 金额口径：SUM(overdue_amt - overdue_adj_amt)，按月份和业务维度聚合并非零过滤
-- 最终输出：overdue_fact
-- ============================================================================
*/
...
WITH
-- 依据当月DETAIL按公司和客商去重性质，供超期款事实回接，避免金额放大。
detail_nature AS (
...
-- OVERDUE：标准化为窄事实接口，账龄段固定为1并回接性质。
overdue_fact AS (
...
-- 来源级Sum聚合：按全部非金额字段汇总OVERDUE账龄金额，结果不是窄事实明细。
overdue_fact_sum AS (
    SELECT dt_month
         , `year`
         , `month`
...
-- 测试目的：核对OVERDUE来源级Sum聚合结果，而非窄事实明细。
SELECT *
  FROM overdue_fact_sum;
